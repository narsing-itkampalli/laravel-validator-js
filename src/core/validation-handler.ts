...
export default class ValidationHandler {
    ...
    public readonly handlers:Partial<ValidationHandlers> = {
        accepted: () => {
        ...
        // JavaScript in the browser cannot directly query DNS records. So it checks just format of the inputValue.
        active_url: () => {
            return ({value, path}) => {
                return {
        ...
        alpha: ({ruleParams}) => {
            const regex = (
                ...
                ruleParams[0] === 'ascii' ? /^[a-zA-Z]+$/ :
    
                // Unicode alphabetic characters (\p{L} includes all letters, \p{M} includes marks)
                /^[\p{L}\p{M}]+$/u
            );
        ...
        alpha_dash: ({ruleParams}) => {
            const regex = (
                // ASCII alphabetic characters: a-z and A-Z
                ruleParams[0] === 'ascii' ? /^[a-zA-Z0-9\-_]+$/ :
        ...
        between: ({ruleParams, fieldRulesArray}) => {
            return ({value, path}) => {
                ...
                const response = {
                    passed: false,
                    type: isInteger ? 'numeric' : (isArray ? 'array' : (valueType === 'file' ? 'file' : 'string')) as ValidatorHandlersReturnType['type'],
                    placeholder: {
                        attribute: path.join('.'),
                        min: ruleParams[0],
                        max: ruleParams[1]
                ...
                if(valueType) {
                    ...
                    if(isNaN(min)) throw new Error(`The given value "${ruleParams[0]}" does not represent a valid number.`);
                    if(isNaN(max)) throw new Error(`The given value "${ruleParams[1]}" does not represent a valid number.`);
        ...
        different: ({ruleParams, fieldValue}) => {
            ...
            return ({value, key, path}) => {
                if(!Array.isArray(fieldValue) && Array.isArray(otherFieldValue)) return {passed: true};
                ...
                if(Array.isArray(otherFieldValue)) {
                    ...
                    if(!otherFieldValueItem) response.passed = true;
                    else response.placeholder.other = otherFieldValueItem.key;
                }else {
        ...
        dimensions: ({ruleParams}) => {
            ...
            return async ({value, path}) => {
                ...
                if(value instanceof File && value.type.startsWith('image/')) {
                    try {
                        const isValidDimention:boolean = await new Promise((resolve, reject) => {
                            const img = new Image();
                            ...
                            img.onload = () => {
                                ...
                                const result = imageRules.every(([ruleName, ruleValue]) => {
                                    if(ruleName === 'min_width' && width < parseFloat(ruleValue)) return false;
                                    if(ruleName === 'max_width' && width > parseFloat(ruleValue)) return false;
                                    if(ruleName === 'min_height' && height < parseFloat(ruleValue)) return false;
                                    if(ruleName === 'max_height' && height > parseFloat(ruleValue)) return false;
                                    if(ruleName === 'width' && width != parseFloat(ruleValue)) return false;
                                    if(ruleName === 'height' && width != parseFloat(ruleValue)) return false;
                                    if(ruleName === 'ratio') {
        ...
        distinct: ({fieldValue, ruleParams}) => {
            ...
            // Validates whether all items in an array are unique.
            // - "strict": Ensures both type and value are checked for uniqueness.
            // - "ignore_case": Ensures case-insensitive uniqueness (applies only to strings and implies "strict").
            // - Default: Checks for uniqueness based only on value (not type).
    
            let ignoreCase = false;
            let strictComparison = false;
    
            if(ruleParams.length) {
                if (ruleParams.includes('strict')) strictComparison = true;
                if (ruleParams.includes('ignore_case')) {
                    ignoreCase = true;
                    strictComparison = true; // "ignore_case" implies "strict".
                }
            }
    
            return ({value, key, path}) => {
                if(!Array.isArray(fieldValue)) return {passed: true};
    
                const passed = !fieldValue.find((fieldValueItem) => {
                    if(fieldValueItem.key == key) return false;
        ...
        email: ({ruleParams}) => {
            // DNS validation is not possible due to browser restrictions.
            // The "filter" rule is omitted as it's unnecessary on the frontend 
            // (to avoid revealing which emails are blocked).
    
            const validationFlags:{[key:string]:boolean} = {
            ...
            return ({value, path}) => {
                const isValidEmail = () => {
                    ...
                    if (!EmailValidator.isRFCValid(value)) return false; // Basic RFC validation.
                    if (validationFlags.strict && !EmailValidator.isStrictValid(value)) return false; // Strict validation.
                    if (validationFlags.spoof && EmailValidator.isSpoofedDomain(value)) return false; // Spoofed domain check.
                    if (validationFlags.filter_unicode && !EmailValidator.isUnicodeFiltered(value)) return false; // Unicode filtering.
        ...
        gt: ({ruleParams, fieldRulesArray}) => {
            ...
            return ({value, path}) => {
                ...
                const response = {
                    passed: false,
                    type: (isInteger || valueType == 'numeric') ? 'numeric' : (isArray ? 'array' : (valueType === 'file' ? 'file' : 'string')) as ValidatorHandlersReturnType['type'],
                    placeholder: {
                        attribute: path.join('.'),
                        value: ruleParams[0]
                    }
                ...
                if(valueType && !Array.isArray(otherFieldValue)) {
                    ...
                    if(otherFieldValueType) {
                        ...
                        if(valueType === otherFieldValueType) response.passed = valueSize > otherFieldValueSize;
                    }
                }
        ...
        list: () => {
            return ({value, path}) => {
                let passed = false;
                if(Array.isArray(value)) passed = true;
        ...
        max: ({ruleParams, fieldRulesArray}) => {
            return ({value, path}) => {
                ...
                if(valueType) {
                    ...
                    if(isNaN(max)) throw new Error(`The given value "${ruleParams[0]}" does not represent a valid number.`);
        ...
        missing_with: ({ruleParams}) => {
            const withCondition = ruleParams.some(otherField => {
                ...
                if(Array.isArray(otherFieldValue)) return false;
        ...
        not_regex: ({fieldName, ruleParams}) => {
            ...
            if(!extractedRegex) throw new Error(`Invalid Regex provided for the field ${fieldName}`);
        ...
        // Laravel return true if value is an uploaded file with an empty path.
        prohibited: () => {
            return ({value, path}) => {
                let passed = false;
    
                if(value === undefined || (typeof value === 'string' && !value.trim())) passed = true;
                else if(value === null) passed = true;
                else if(Array.isArray(value) || isPlainObject(value)) passed = Object.keys(value).length === 0;
        ...
        prohibits: ({ruleParams}) => {
            ...
            const areAllOtherFieldEmpty = ruleParams.every(otherField => {
                const otherFieldValue = this.context.data.get(otherField);
                if(Array.isArray(otherFieldValue)) return true;
        ...
        size: ({ruleParams, fieldRulesArray}) => {
            return ({value, path}) => {
                ...
                if(valueType) {
                    ...
                    if(isNaN(size)) throw new Error(`The given value "${ruleParams[0]}" does not represent a valid number.`);
    ...
    private readonly helpers = {
        if: (ruleParams:ValidatorRulesArray[number]['params']) => {
            const [otherFieldName, ...definedValues] = ruleParams;

            const dataResult = this.context.data.get(otherFieldName);

            if(Array.isArray(dataResult)) return {passed: false};

            // Prepare rule values with both original and enriched formats
            const enrichedRuleValues = definedValues.flatMap((item: any) => {
                if (item === 'true') return [item, true];
                if (item === 'false') return [item, false];
                if (/^[0-9]+$/.test(item)) return [item, parseInt(item, 10)];
                return [item];
            });

            // Compare the parsed rule values with the value
        ...
        isEmpty: (value:any) => {
            ...
            else if(Array.isArray(value) || isPlainObject(value)) return Object.keys(value).length === 0;
            return false;
        },
        extractRegexFromString: (string:string) => {
        ...
        isAccepted: (value:any) => {
            return ['yes', 'on', 1, '1', true, 'true'].includes(value);
        },
        isDeclined: (value:any) => {
            return ['no', 'off', 0, '0', false, 'false'].includes(value);
        },
        getDataType: (value:any):ValidatorHandlersReturnType['type']|null => {
            if(Array.isArray(value) || isPlainObject(value)) return 'array';
            if(['string', 'number'].includes(typeof value)) {
        ...
        getSize: (value:any, valueType:ValidatorHandlersReturnType['type'], isInteger:boolean) => {
            if(valueType === 'string' || valueType === 'numeric') {
                return (isInteger && valueType === 'numeric') ? Number(value) : String(value).length;
            }else if(valueType === 'array') {
                return Object.keys(value).length;
            }
            return value.size / 1000; // File size in kilobytes
        }
    }
}